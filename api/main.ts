...
app.get('/mysql/extrato',async  (req: Request, res: Response) => {
  ...
  const token:any = await makeQuery('SELECT *  as resultados FROM comparativo.chave_valor  where  expiration <= STR_TO_DATE( \'' +hour +'\'  , \'%Y-%m-%d %H:%i:%s\') LIMIT 1 ')
  actions.push('[MYSQL]: Get Token')
  console.log('veio do db')
  console.log(token[0]['resultados']['valor'])
  if (token[0]['valor']){
